  ...
  <div className="flex flex-wrap gap-3 justify-center">
    {timeSlots.map(([start, end]) => {
      ...
      return (
        <div
          key={start}
          onClick={() => status === 'available' && handleClick(start)}
          className={`w-28 rounded-xl p-3 flex flex-col items-center justify-center text-center gap-1 cursor-pointer transition ${
            status === 'available'
              ? 'bg-green-500 text-white hover:bg-green-600'
              : 'bg-red-500 text-white cursor-not-allowed'
          }`}
        >
          <div className="text-xs font-semibold">{start} - {end}</div>
